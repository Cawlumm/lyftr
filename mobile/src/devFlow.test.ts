...
describe('eas build profiles', () => {
  ...
  it('ships production to the store, not as a sideloadable internal build', () => {
    expect(easJson.build.production.distribution).toBe('store')
    expect(easJson.build.production.android.buildType).toBe('app-bundle')
  })

  // Channels are consumed by expo-updates. Declaring one without that package
  // installed is config that reads as meaningful and does nothing — it produced a
  // warning on every build for months. If updates get adopted, delete this test
  // along with the omission it guards.
  it('does not declare update channels while expo-updates is absent', () => {
    const pkg = JSON.parse(readFileSync(join(MOBILE, 'package.json'), 'utf8'))
    const hasUpdates = 'expo-updates' in { ...pkg.dependencies, ...pkg.devDependencies }
