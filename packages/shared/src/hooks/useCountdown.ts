...
// JS-timer pauses (backgrounding on native) — on foreground the next tick recomputes
// from `endsAt`. Returns whole seconds remaining (>= 0), or null when `endsAt` is
// null. Fires `onComplete` exactly once when it first reaches 0.
