...
func (s *FoodStore) ListByDay(uid int64, date string) ([]models.FoodLog, error) {
	rows, err := s.db.Query(
		foodLogSelect+` WHERE user_id = ? AND logged_on = ? ORDER BY logged_at ASC`,
		uid, date,
	)
	if err != nil {
		return nil, err
...
func (s *FoodStore) Create(uid int64, req models.LogFoodRequest) (models.FoodLog, error) {
	res, err := s.db.Exec(
		...
		req.Servings, req.ServingSize, req.Barcode, req.ImageURL, req.LoggedAt, req.LoggedOn,
	)
	if err != nil {
...
//
// Grouping is a plain GROUP BY on the stored day. Before logged_on existed this
// had to group on substr(logged_at, 1, 10) — the UTC date — which put entries on a
// different day than the daily totals shown above the same chart.
func (s *FoodStore) History(uid int64, cutoff string) ([]models.FoodHistoryPoint, error) {
	rows, err := s.db.Query(
		`SELECT logged_on,
