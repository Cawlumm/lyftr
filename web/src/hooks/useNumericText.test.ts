...
describe('useNumericText', () => {
  ...
  it('re-syncs when the parent value is a genuinely different number (stepper/prefill)', () => {
    const { result, rerender } = renderHook(({ v }) => useNumericText(v), { initialProps: { v: '90' } })
    rerender({ v: '95' }) // stepper / programmatic change / deferred prefill
    expect(result.current[0]).toBe('95')
  })

  it('populates a deferred prefill (empty → value), e.g. autofocus edit modals', () => {
    const { result, rerender } = renderHook(({ v }) => useNumericText(v), { initialProps: { v: '' } })
    rerender({ v: '180' }) // parent effect prefills after mount
